...
import { personaValueAt } from "./results-panel.js";
...
export function createDrilldownRenderer({
  ...
  function render(state) {
    ...
    const overlayActive = Number.isInteger(state.overlayIndex)
      && state.overlayIndex >= 0
      && state.overlayIndex < (state.results?.n ?? 0)
      && state.results?.personaCodes instanceof Uint32Array;
    const sampledNodeIds = overlayActive
      ? new Set(Array.isArray(state.results.personaNodeIds) ? state.results.personaNodeIds : [])
      : null;
    ...
    for (const node of view.nodes) {
      ...
      let overlay = null;
      if (overlayActive) {
        if (!sampledNodeIds.has(node.id)) {
          overlay = { visible: "Not sampled", accessible: "not sampled in this result" };
        } else {
          const value = personaValueAt(
            state.results,
            state.store.core,
            state.overlayIndex,
            node.id,
          ...
          overlay = value === null
            ? { visible: "Value unavailable", accessible: "persona value unavailable" }
            : { visible: `Persona: ${value}`, accessible: `persona value ${value}` };
        }
      }
      const facts = [
        node.label,
      ...
      const meta = withText(svgElement("text", {
        class: "syn-dd-meta",
        x: "8",
        y: "27",
        "fill-opacity": "0.64",
